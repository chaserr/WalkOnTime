...
- (void)layoutMainTableViewCell{
    ...
    // 标题
    self.titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(_titleView.frame)-100,CGRectGetHeight(_titleView.frame))];
...
- (void)tapGestureWithTitle:(UITapGestureRecognizer *)tapGuesture {
    ...
    if ([_delegate respondsToSelector:@selector(txPhotoTableViewCellShowDetailWithTapGesture:txPhotoListModelWithUrl:)]) {
        [_delegate txPhotoTableViewCellShowDetailWithTapGesture:tapGuesture txPhotoListModelWithUrl:self.txPhotoListModel.subject_collection.subject_collection_url];
    }
...
- (void)tapGestureWithfirstImg:(UITapGestureRecognizer *)tapGuesture {
    ...
    NSArray *array = self.txPhotoListModel.subjects;
    ...
    if ([_delegate respondsToSelector:@selector(txPhotoTableViewCellShowDetailWithTapGesture:txPhotoListModelFirstImgWithUrl:)]) {
        [_delegate txPhotoTableViewCellShowDetailWithTapGesture:tapGuesture txPhotoListModelFirstImgWithUrl:urlString];
    }
...
//    NSString *normalImgUrl = [subjectsDict objectForKey:@"id"];
    ...
    if ([_delegate respondsToSelector:@selector(txPhotoTableViewCellShowDetailWithTapGesture:txPhotoListModelSecondImgWithUrl:)]) {
        [_delegate txPhotoTableViewCellShowDetailWithTapGesture:tapGuesture txPhotoListModelSecondImgWithUrl:urlString];
    }
...
//    NSString *normalImgUrl = [subjectsDict objectForKey:@"id"];
    ...
    SubjectsModel *subjectModel = array[2];
    NSString *urlString = subjectModel.itemId;
    if ([_delegate respondsToSelector:@selector(txPhotoTableViewCellShowDetailWithTapGesture:txPhotoListModelThirdImgWithUrl:)]) {
        [_delegate txPhotoTableViewCellShowDetailWithTapGesture:tapGuesture txPhotoListModelThirdImgWithUrl:urlString];
    }
    
}


- (void)setTxPhotoListModel:(TXPhotoListModel *)txPhotoListModel{

    _txPhotoListModel = txPhotoListModel;
    
    
    
    // 赋值
    
    self.titleLabel.text = txPhotoListModel.subject_collection.name;
    
    NSString *lookNumber = [NSString stringWithFormat:@"%@", txPhotoListModel.subject_collection.subject_count];
    
    self.lookCount.text = lookNumber;
    
    
    
    
    NSArray *array = txPhotoListModel.subjects;
