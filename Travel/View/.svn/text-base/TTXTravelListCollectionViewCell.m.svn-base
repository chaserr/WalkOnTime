//
//  TTXTravelListCollectionViewCell.m
//  WalkOnTime
//
...
@implementation TTXTravelListCollectionViewCell
...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _collectionImgView.backgroundColor = [UIColor colorWithRed:0.213 green:0.667 blue:0.489 alpha:1.000];
...
- (void)setTxDoulistModel:(TXDoulistsModel *)txDoulistModel{

    _txDoulistModel = txDoulistModel;
    
    // 赋值
    self.titleLabel.text = txDoulistModel.title;
    
    NSString *flowCountLableString = [NSString stringWithFormat:@"%@人关注", txDoulistModel.followers_count];
    
    self.flowCountLable.text = flowCountLableString;/*txDoulistModel.followers_count;*/
    [self.collectionImgView sd_setImageWithURL:[NSURL URLWithString:txDoulistModel.merged_cover_url]];
